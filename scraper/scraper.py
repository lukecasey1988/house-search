...
# Build a lookup from (source, source_id) -> group index, so we can quickly
# check if two listings were manually confirmed as duplicates in config.py.
_MANUAL_GROUP_LOOKUP = {}
for _group_idx, _group in enumerate(MANUAL_DUPLICATE_GROUPS):
    for _entry in _group:
        _MANUAL_GROUP_LOOKUP[(_entry["source"], str(_entry["source_id"]))] = _group_idx
...
def is_likely_duplicate(a: dict, b: dict) -> bool:
    """Duplicate check with four routes to a match, in order of confidence:

    0. Manually confirmed in config.py's MANUAL_DUPLICATE_GROUPS - always
       wins, since a human directly confirmed it.
    ...
    1. Perceptual image hash match (any sources) - the two thumbnails are the
       ...
       main automatic mechanism and works across different sites/domains.
    ...
    houses into one.
    """
    key_a = (a["source"], str(a["source_id"]))
    key_b = (b["source"], str(b["source_id"]))
    if key_a in _MANUAL_GROUP_LOOKUP and key_b in _MANUAL_GROUP_LOOKUP:
        if _MANUAL_GROUP_LOOKUP[key_a] == _MANUAL_GROUP_LOOKUP[key_b]:
            return True
...
def deduplicate(listings: list[dict]) -> list[dict]:
    ...
    Groups are built by checking a new listing against EVERY existing member
    of a group, not just the first one added - this matters for 3+ way
    duplicates where, say, listing A matches listing B via one method (e.g.
    image hash) and listing C matches listing A via a different method (e.g.
    a manual override), but C and B don't directly match each other by any
    method. Checking against only the group's first member would miss this
    and split what should be one merged card into two.
    """
    ...
    for listing in listings:
        matched_group = None
        for group in groups:
            if any(is_likely_duplicate(member, listing) for member in group):
                matched_group = group
                break
        ...
        if matched_group is None:
            ...
            matched_group.append(listing)

    merged = []
    for group in groups:
        primary = dict(group[0])
        primary["sources"] = [{"name": m["source"], "url": m["url"]} for m in group]
        for m in group[1:]:
            if m.get("address") and len(m["address"]) > len(primary.get("address") or ""):
                primary["address"] = m["address"]
            if m.get("summary") and not primary.get("summary"):
                primary["summary"] = m["summary"]
            if m.get("thumbnail") and not primary.get("thumbnail"):
                primary["thumbnail"] = m["thumbnail"]
            if m.get("bedrooms") and not primary.get("bedrooms"):
                primary["bedrooms"] = m["bedrooms"]
            if m.get("bathrooms") and not primary.get("bathrooms"):
                primary["bathrooms"] = m["bathrooms"]
